...
func toString() -> String:
	var template = "Cell data: {position} TileName: {tileName} Avoidance: {avd} DefBonus: {def} MoveCost: {mvd} OccupyingUnit: {oUnit}"
	return template.format({
		"position": str(cellPosition),
		"tileName": tileName,
		"avd": avoidanceBonus,
		"def": defenseBonus,
		"mvd": movementCost,
		"oUnit": str(occupyingUnit) if occupyingUnit else "None"
	})
